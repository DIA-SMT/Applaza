...
const layerDefinitions = [
  { id: "plaza", label: "Plazas", group: "Espacios", color: "#0166ff" },
  { id: "espacio_verde", label: "Espacios verdes", group: "Espacios", color: "#16a34a" },
  { id: "platabanda", label: "Platabandas", group: "Espacios", color: "#8b5cf6" },
...
const statusLayerIds = new Set(["en_curso", "finalizado", "vencido", "observado"]);
...
export function OperationalMap({ spaces, providers, currentUser, dataError, setSpaces }: { spaces: SpaceRecord[]; providers: Provider[]; currentUser: UserProfile; dataError: string | null; setSpaces: Dispatch<SetStateAction<SpaceRecord[]>> }) {
  ...
  const [query, setQuery] = useState(""); const [providerId, setProviderId] = useState("all"); const [status, setStatus] = useState("all"); const [type, setType] = useState("all"); const [neighborhood, setNeighborhood] = useState("all");
  const [startDate, setStartDate] = useState(""); const [endDate, setEndDate] = useState("");
  const [activeLayers, setActiveLayers] = useState(() => new Set(layerDefinitions.map((layer) => layer.id as string)));
  ...
  const filteredSpaces = useMemo(() => spaces.filter((space) => {
    ...
    const matchesText = !term || space.name.toLocaleLowerCase("es").includes(term) || space.address?.toLocaleLowerCase("es").includes(term) || space.neighborhood?.toLocaleLowerCase("es").includes(term);
    const typeLayerVisible = activeLayers.has(space.type);
    const statusLayerVisible = !statusLayerIds.has(space.status) || activeLayers.has(space.status);
    const matchesDates = (!startDate || Boolean(task && task.end_date >= startDate)) && (!endDate || Boolean(task && task.start_date <= endDate));
    return matchesText && typeLayerVisible && statusLayerVisible && (providerId === "all" || space.provider?.id === providerId) && (status === "all" || space.status === status) && (type === "all" || space.type === type) && (neighborhood === "all" || space.neighborhood === neighborhood) && matchesDates;
  }), [spaces, query, activeLayers, providerId, status, type, neighborhood, startDate, endDate]);
  ...
  const activeFilterCount = [providerId, status, type, neighborhood].filter((value) => value !== "all").length + Number(Boolean(query)) + Number(Boolean(startDate)) + Number(Boolean(endDate));

  function layerCount(id: string) { return spaces.filter((space) => space.type === id || space.status === id).length; }
  ...
  function clearFilters() { setQuery(""); setProviderId("all"); setStatus("all"); setType("all"); setNeighborhood("all"); setStartDate(""); setEndDate(""); }
  ...
  return <section className="gis-workspace">
    <header className="gis-toolbar">
      <div className="gis-search"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar plaza, dirección o barrio" />{query && <button onClick={() => setQuery("")} aria-label="Limpiar búsqueda"><X size={15} /></button>}</div>
      ...
      {canEditLocations && <button className={`gis-location-button ${locationMode ? "active" : ""}`} onClick={() => { setLocationMode((value) => !value); setSelectedId(undefined); setDraftLocation(undefined); }}><Crosshair size={17} />{locationMode ? "Cerrar editor" : `Ubicar (${pendingSpaces.length})`}</button>}
    </header>
    ...
    <div className={`gis-filter-drawer ${filtersOpen ? "open" : ""}`}>
      ...
      <label>Tipo<select value={type} onChange={(event) => setType(event.target.value)}><option value="all">Todos</option><option value="plaza">Plaza</option><option value="espacio_verde">Espacio verde</option><option value="platabanda">Platabanda</option></select></label>
