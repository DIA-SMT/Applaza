...
create policy "Staff update locations" on public.green_spaces for update to authenticated using (public.current_user_role()::text in ('admin','supervisor','inspector')) with check (public.current_user_role()::text in ('admin','supervisor','inspector'));
drop policy if exists "Staff manage tasks" on public.maintenance_tasks;
create policy "Staff manage tasks" on public.maintenance_tasks for all to authenticated using (public.current_user_role()::text in ('admin','supervisor','inspector')) with check (public.current_user_role()::text in ('admin','supervisor','inspector'));
